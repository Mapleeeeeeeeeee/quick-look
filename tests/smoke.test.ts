...
const BINARY_PATH = path.resolve(__dirname, '../src-tauri/target/release/copilot-preview');
const BINARY_EXISTS = existsSync(BINARY_PATH);

describe.skipIf(!BINARY_EXISTS)('app smoke tests', () => {
  ...
  afterEach(async () => {
    ...
    appProcess = null;
    try { execSync('pkill -f copilot-preview 2>/dev/null'); } catch {}
    await new Promise(resolve => setTimeout(resolve, 500));
  });

  afterAll(() => {
    try { execSync(`rm -rf ${TEST_DIR}`); } catch {}
  });
