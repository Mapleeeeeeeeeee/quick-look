...
describe("show_file tool integration", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.stubEnv("QUICK_LOOK_BINARY", "/mock/binary");
  ...
  describe("Journey 2: line range", () => {
    it("passes start and end line flags to spawn and includes range in message", async () => {
      ...
      expect(mockSpawn).toHaveBeenCalledWith(
        "/mock/binary",
        ["/proj/utils.ts", "--start-line", "50", "--end-line", "80"],
        { detached: true, stdio: "ignore" },
      );
      expect(result).toBe(
        "Opened preview: utils.ts (200 lines), highlighting lines 50-80",
      );
    });
  });
  ...
  describe("Journey 3: consecutive calls", () => {
    it("spawns with correct path for each call and returns correct messages", async () => {
      ...
      expect(mockSpawn).toHaveBeenCalledWith("/mock/binary", ["/proj/a.ts"], {
        detached: true,
        stdio: "ignore",
      });
      expect(mockSpawn).toHaveBeenCalledWith("/mock/binary", ["/proj/b.ts"], {
        detached: true,
        stdio: "ignore",
      });
    });
