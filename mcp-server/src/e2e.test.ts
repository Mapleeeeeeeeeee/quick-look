...
class McpTestClient {
  ...
  constructor() {
    ...
    this.process = spawn("node", [serverPath], {
      ...
      env: {
        ...process.env,
        QUICK_LOOK_BINARY: "/tmp/fake-quick-look",
...
const FAKE_BINARY = "/tmp/fake-quick-look";
...
beforeAll(() => {
  ...
  writeFileSync(path.join(TEST_DIR, "README.md"), "# Test\n\nHello world\n");
...
describe("MCP server E2E", () => {
  ...
  it("responds to initialize with server info", async () => {
    ...
    expect(result.serverInfo.name).toBe("quick-look");
